...
if [ -z "$PATCHES_DIR" ]; then
  ...
  elif [ -d "$HOME/.antigravitycli/gsd-local-patches" ]; then
    PATCHES_DIR="$HOME/.antigravitycli/gsd-local-patches"
  elif [ -d "$HOME/.codex/gsd-local-patches" ]; then
    PATCHES_DIR="$HOME/.codex/gsd-local-patches"
  else
    PATCHES_DIR="/Users/shaleensharma/workplace/gemini_ws/portfolio-tracker/.antigravitycli/gsd-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .antigravitycli .codex .claude; do
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `/Users/shaleensharma/workplace/gemini_ws/portfolio-tracker/.antigravitycli/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
