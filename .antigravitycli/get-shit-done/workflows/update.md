...
- Path contains `/.antigravitycli/` -> `gemini`
...
RUNTIME_DIRS=( "claude:.claude" "opencode:.config/opencode" "opencode:.opencode" "gemini:.antigravitycli" "kilo:.config/kilo" "kilo:.kilo" "codex:.codex" )
ENV_RUNTIME_DIRS=()
...
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/get-shit-done/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/get-shit-done/workflows/update.md" ]; }; then
  ...
  for dir in .claude .config/opencode .opencode .antigravitycli .config/kilo .kilo .codex; do
...
- Line 4 = resolved GSD config dir (e.g. `/Users/me/.claude`, `/Users/me/.antigravitycli`); empty if scope is `UNKNOWN`. Capture this as `GSD_DIR` and pass it to subsequent steps so they don't have to re-derive the runtime path.
...
The `GSD_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`/Users/shaleensharma/workplace/gemini_ws/portfolio-tracker/.antigravitycli/`, `~/.antigravitycli/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `GSD_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.
...
global: `/Users/shaleensharma/workplace/gemini_ws/portfolio-tracker/.antigravitycli/`, `~/.config/opencode/`, `~/.opencode/`, `~/.antigravitycli/`, `~/.config/kilo/`, or `~/.codex/`
local: `./.antigravitycli/`, `./.config/opencode/`, `./.opencode/`, `./.antigravitycli/`, `./.kilo/`, or `./.codex/`)
...
```bash
# RUNTIME_DIR is the resolved config directory (e.g. ~/.config/opencode, ~/.antigravitycli)
